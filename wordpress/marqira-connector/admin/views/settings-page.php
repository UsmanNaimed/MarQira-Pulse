...
<div class="wrap marqira-wrap">
        ...
        <div class="marqira-grid">
                ...
                <div class="marqira-col-side">
                        ...
                        <div class="marqira-card">
                                <h2 class="marqira-section-title"><?php esc_html_e( 'Connection Status', 'marqira-connector' ); ?></h2>
                                
                                <?php if ( $enrolled ) : ?>
                                        <!-- Enrolled -->
                                        <div class="marqira-connection-enrolled">
                                                <p class="marqira-connection-status">
                                                        <span class="dashicons dashicons-yes-alt" style="color: #46b450;"></span>
                                                        <strong><?php esc_html_e( 'Connected to MarQira', 'marqira-connector' ); ?></strong>
                                                </p>
                                                <table class="marqira-diagnostics-table">
                                                        <tbody>
                                                                <tr>
                                                                        <th><?php esc_html_e( 'Site UUID', 'marqira-connector' ); ?></th>
                                                                        <td>
                                                                                <code><?php echo esc_html( ! empty( $credentials['site_uuid'] ) ? substr( $credentials['site_uuid'], 0, 8 ) . '...' : '—' ); ?></code>
                                                                        </td>
                                                                </tr>
                                                                <tr>
                                                                        <th><?php esc_html_e( 'Enrolled', 'marqira-connector' ); ?></th>
                                                                        <td><?php echo esc_html( ! empty( $credentials['enrolled_at'] ) ? $credentials['enrolled_at'] : '—' ); ?></td>
                                                                </tr>
                                                                <tr>
                                                                        <th><?php esc_html_e( 'Last Heartbeat Sent', 'marqira-connector' ); ?></th>
                                                                        <td>
                                                                                <?php
                                                                                if ( $last_heartbeat ) {
                                                                                        /* translators: %s: time ago */
                                                                                        printf( esc_html__( '%s ago', 'marqira-connector' ), esc_html( human_time_diff( $last_heartbeat ) ) );
                                                                                } else {
                                                                                        esc_html_e( 'Never', 'marqira-connector' );
                                                                                }
                                                                                ?>
                                                                        </td>
                                                                </tr>
                                                                <tr>
                                                                        <th><?php esc_html_e( 'API URL', 'marqira-connector' ); ?></th>
                                                                        <td><code><?php echo esc_html( ! empty( $credentials['api_url'] ) ? $credentials['api_url'] : '—' ); ?></code></td>
                                                                </tr>
                                                        </tbody>
                                                </table>
                                                ...
                                                <form method="post" action="<?php echo esc_url( $save_url ); ?>" style="margin-top: 15px;">
                                                        ...
                                                        <button type="submit" class="button" onclick="return confirm('<?php esc_attr_e( 'Are you sure you want to disconnect this site from MarQira? Heartbeats will stop.', 'marqira-connector' ); ?>');">
                                                                <?php esc_html_e( 'Disconnect', 'marqira-connector' ); ?>
                                                        </button>
                                                </form>
                                        </div>
                                <?php else : ?>
                                        <!-- Not Enrolled -->
                                        <div class="marqira-connection-not-enrolled">
                                                <p class="marqira-connection-status">
                                                        <span class="dashicons dashicons-warning" style="color: #f0b849;"></span>
                                                        <strong><?php esc_html_e( 'Not Connected', 'marqira-connector' ); ?></strong>
                                                </p>
                                                <p><?php esc_html_e( 'To connect this site to MarQira Pulse, paste your enrollment token below.', 'marqira-connector' ); ?></p>

                                                <form method="post" action="<?php echo esc_url( $save_url ); ?>">
                                                        <?php wp_nonce_field( 'marqira_enroll', 'marqira_enroll_nonce' ); ?>
                                                        <input type="hidden" name="action" value="marqira_enroll" />
                                                        
                                                        <table class="form-table">
                                                                <tr>
                                                                        <th><label for="enrollment_token"><?php esc_html_e( 'Enrollment Token', 'marqira-connector' ); ?></label></th>
                                                                        <td>
                                                                                <input type="text" 
                                                                                           id="enrollment_token" 
                                                                                           name="enrollment_token" 
                                                                                           class="regular-text code" 
                                                                                           placeholder="MQ-CONNECT-..." 
                                                                                           required />
                                                                                <p class="description">
                                                                                        <?php esc_html_e( 'Get this token from your MarQira dashboard.', 'marqira-connector' ); ?>
                                                                                </p>
                                                                        </td>
                                                                </tr>
                                                        </table>
                                                        ...
                                                        <p class="submit">
                                                                <button type="submit" class="button button-primary">
                                                                        <?php esc_html_e( 'Connect to MarQira', 'marqira-connector' ); ?>
                                                                </button>
                                                        </p>
                                                </form>
                                        </div>
