...
class Marqira_App_Password_Guard {
        ...
        public function handle_app_password_auth_errors( $error, $user, $item, $request ) {
                ...
                // Phase 4: Use dynamically fetched allowed IPs (with fallback to settings)
                $allowed_ips = Marqira_Config_Fetcher::get_allowed_ips();
                
                // Fallback to settings if config fetcher returns empty
